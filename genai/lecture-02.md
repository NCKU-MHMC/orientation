...
<!--
[約 3 分鐘] 從第一堂結尾留下的問題接上。

第一堂寫過的每一種損失,KL 系列、DPO、DDO 的判別器,拆開來看都要呼叫 logprob。
那麼一個只會生樣本、不會報密度的模型,要拿什麼訓練?

今天的任務:用神經網路構造出提供這兩個介面的物件。
AR、Flow、VAE、EBM、DPM、GAN 各是一種答案。
...
**應用**:對話與 agent、程式生成,以及 LLM-ASR 裡外掛語言模型的角色(shallow fusion 的 LM 項)。
...
同一條 chain rule,擺在資料側或雜訊側,兩個介面的成本表整個對調。Parallel WaveNet 的解法:MAF 型 teacher 高效訓練,蒸餾給 IAF 型 student;student 對自己生成的樣本算 logprob 是並行的($u$ 已知),兩側的快同時到手([van den Oord et al., 2018](https://arxiv.org/abs/1711.10433))。

</div>

<!--
同一條 chain rule,擺在資料側或雜訊側,兩個介面的成本表整個對調。

MAF(Papamakarios et al., 2017):自迴歸依賴放在資料側,
logprob 一次並行前向,快;sample 逐維串行,O(D),慢。適合密度估計與 MLE 訓練。
IAF(Kingma et al., 2016):依賴放在雜訊側,
sample 一次抽 u 就並行生成,快;logprob 要逐維還原 u,慢。適合即時生成與變分後驗。

Parallel WaveNet 的解法值得記住:MAF 型的 teacher 高效訓練,蒸餾給 IAF 型的 student。
關鍵是 student 對自己生成的樣本算 logprob 是並行的,因為 u 已經知道了,
於是兩側的快同時到手(van den Oord et al., 2018)。
